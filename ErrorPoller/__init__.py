...
For each new failure:
  ...
  3. Claude Sonnet analysis with full context
...
def main(timer: func.TimerRequest) -> None:
    ...
    for error_row in new_errors:
        ...
        # 5. Claude
        try:
            ai_suggestion = analyze_with_claude(
                error_details=full_context,
                service=service,
                run_id=run_id,
                rag_context=rag_context,
            )
            logger.info("Claude done — Priority=%s Confidence=%s",
                        ai_suggestion.get("priority"),
                        ai_suggestion.get("confidence"))
        except Exception as e:
            logger.error("Claude failed for error_id=%d: %s", error_id, e)
...
def _build_full_context(row: dict, parsed: dict | None) -> str:
    """
    Full context passed to Claude for structured analysis.
    For runlog rows, unpacks the JSON to give Claude named fields.
    """
    parts = [
        f"Source: {row['source']}",
    ...
    if parsed:
        # Structured orchestrator error — give Claude the parsed fields
        parts += [
